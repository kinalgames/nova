...
describe('markdown rendering in text blocks', () => {
  it('renders headings, emphasis, lists, blockquote, GFM table and links', async () => {
    await seedConv(FULL_MD)()

    // generous timeout: the lazy Markdown chunk loads slower under coverage instrumentation
    expect(await screen.findByText('Tiêu đề lớn', undefined, { timeout: 5000 })).toBeInTheDocument()
  ...
  it('renders a code fence as a card — language header, highlight, working copy', async () => {
    const user = makeUser()
    ...
    expect(await screen.findByText('ts', undefined, { timeout: 5000 })).toBeInTheDocument()
  ...
  it('keeps short inline code out of the card treatment', async () => {
    await seedConv('Gõ `ls -la` để xem.')()
    const inline = await screen.findByText('ls -la', undefined, { timeout: 5000 })
